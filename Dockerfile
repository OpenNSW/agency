...
#
# Runtime config (/config.js, window.__APP_CONFIG__) is generated by the Go
# server per request from its own config.yaml (web.runtime) — no entrypoint
# script, no file is written at runtime.
...
# /config.js is generated dynamically from config.yaml (web.runtime), so
# nothing is written into this directory at runtime.
