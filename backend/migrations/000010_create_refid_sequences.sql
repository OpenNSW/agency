...
-- resolved scope key). Created here rather than via refid's own Migrate
-- helpers so the .sql file stays the single source of truth for the schema
-- (docs/migrations.md) and the table gets down/status like every other one.
-- Keep the shape identical to those helpers' DDL — refid's queries run
