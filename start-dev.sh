...
# name, IdP client id/scopes, expected OU, ...) served at /config.js, are NOT
# generated by this script — they're the checked-in
# backend/config/<agency>/config.yaml's web.runtime section (see
...
# Orchestration-only per-agency config: "BE_PORT|FE_PORT|APP_NAME".
# Everything else the frontend needs at runtime (branding name, IdP client
# id/OU handle/scopes, ...) lives in backend/config/<agency>/config.yaml's
# web.runtime section instead — the frontend's Vite dev server proxies
# /config.js to its paired backend to read it (see frontend/vite.config.ts),
# same as prod. This table only feeds the frontend dev server's own port and
# this script's own logging, and BE_PORT/FE_PORT here must match that agency's
...
CONFIG_npqs="8081|5174|National Plant Quarantine Service (NPQS)"
CONFIG_fcau="8082|5175|Food Control Administration Unit (FCAU)"
CONFIG_cda="8083|5176|Coconut Development Authority (CDA)"
CONFIG_slpa="8084|5177|Sri Lanka Ports Authority (SLPA)"
CONFIG_customs="8085|5178|Sri Lanka Customs (CUSTOMS)"
CONFIG_sltb="8086|5179|Sri Lanka Tea Board (SLTB)"
...
# Sets BE_PORT, FE_PORT, APP_NAME for the given agency (orchestration only —
# see the CONFIG_* table above).
resolve_agency() {
  ...
  IFS='|' read -r BE_PORT FE_PORT APP_NAME <<<"$config"
  APP_NAME="${APP_NAME:-$1}"
}
...
start_frontend() {
  ...
  (
    ...
    # backend's config.yaml web.runtime, not from env vars here anymore.
